...
public class BubbleSprayMain : Spell {
    ...
    IEnumerator ShootBubble() {
        for (int i = 0; i < 10; i++) {
            ...
            Vector3 currMouseVec = (mouseGameObject.transform.position - gunGameObject.transform.position).normalized;
            newBubble.GetComponent<Rigidbody2D>().AddForce(Quaternion.Euler(0, 0, Random.Range(-50, 50)) * currMouseVec * Random.Range(1000, 1700) * speedMultiplier);
